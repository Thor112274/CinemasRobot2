...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1001876618287 -1002009886466').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '').split()]
auth_channel = environ.get('AUTH_CHANNELS', '-1001622914589 -1001948532295')
auth_grp = environ.get('AUTH_GROUP')
#AUTH_CHANNELS = {int(cid.strip()) for cid in auth_channel.split() if cid.strip() and id_pattern.search(cid)}
AUTH_CHANNELS = {int(cid.strip()) for cid in auth_channel.split() if cid.strip() and id_pattern.search(cid)} else None
...
COLLECTION_NAME = environ.get('COLLECTION_NAME', 'CultCinemasDB')
...
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/ML_files')
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1001945547524')).split()]
...
URL = "https://cultcinemas-9fd1a25e62ae.herokuapp.com/".format(FQDN) if ON_HEROKU or NO_PORT else \
    "https://cultcinemas-9fd1a25e62ae.herokuapp.com/".format(FQDN, PORT)
...
if HAS_SSL:
    URL = "https://cultcinemas-9fd1a25e62ae.herokuapp.com/".format(FQDN)
else:
    URL = "https://cultcinemas-9fd1a25e62ae.herokuapp.com/".format(FQDN)
